...
public class Rdaemon {
    ...
    // GLG HACK: Temp files usage has to be enhanced to allow multiple
    // instances of the program using Rsession to not collide if using
	// a common temp dir => Changed Rsession to allow playing with 
	// different temp dirs.
    public void start(String http_proxy, String tmpDirectory) {
        ...
        try {
            if (System.getProperty("os.name").contains("Win")) {
                ...
                RserveArgs.append(" --RS-pidfile \\'" + tmpFile.getPath()  + "\\'");
            }
